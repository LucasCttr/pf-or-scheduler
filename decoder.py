...
Implementa la seccion 10.3.4.4 del documento:
...
def build_agenda(
    ...
    for block, specialty_id in chromosome.items():
        ...
        for p in candidates:
            ...
            # Solo se consume una jornada por dia. La duracion de esa
            # jornada se toma de la capacidad real del quirofano de este
            # bloque (convertida a horas), en lugar de un valor fijo, ya
            # que daily_capacity_minutes puede variar segun la sala.
            if block.day not in surgeon_days_used[surgeon.id]:

                shift_hours = capacity / 60
